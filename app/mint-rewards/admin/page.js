...
export default function AdminPage() {
  ...
  function addPool() {
    const i = pools.length;
    const newPool = {
      id: i,
      label: `Pool ${i + 1}`,
      locked: true,
      status: "locked",
      lockMsg: "This pool is locked. It will open when its turn comes.",
      nfts: [],
    };
  ...
  function removePool(pi) {
    if (pools.length <= 1) return;
    if (!confirm(`Delete ${pools[pi].label}? This cannot be undone.`)) return;
    setPools((prev) => prev.filter((_, i) => i !== pi));
    setActivePool(pi > 0 ? pi - 1 : 0);
  }

  function addPool() {
    const newPool = {
      id: pools.length,
      label: `Pool ${pools.length + 1}`,
      locked: true,
      status: "locked",
      lockMsg: "This pool is locked. It will open when its turn comes.",
      nfts: [],
  ...
  function copyFromPool(fromPi) {
    ...
    setPools((prev) =>
      prev.map((p, i) => (i !== activePool ? p : { ...p, nfts: copiedNfts })),
    );
  }
      ...
      prev.map((p, i) =>
        i === pi
          ? {
              ...p,
              [key]: val,
              locked: key === "status" ? val === "locked" : p.locked,
            }
          : p,
      ),
    );
  ...
  function removeNft(pi, ni) {
    setPools((prev) =>
      prev.map((p, i) =>
        i !== pi ? p : { ...p, nfts: p.nfts.filter((_, j) => j !== ni) },
      ),
    );
  }
        ...
        <div style={{ ...s.tabs, alignItems: "center" }}>
          ...
          <button
            ...
            style={{
              ...
              border: "0.5px dashed rgba(153,69,255,0.4)",
              color: "#9945FF",
              padding: "4px 10px",
              borderRadius: 6,
              cursor: "pointer",
        ...
        {pool && (
          <div style={s.editor}>
            <div style={s.editorHeader}>
              ...
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                <select
                  ...
                  onChange={(e) => {
                    if (e.target.value !== "") {
                      copyFromPool(parseInt(e.target.value));
                      e.target.value = "";
                    }
                  }}
                  style={{
                ...
                <select
                  ...
                  onChange={(e) =>
                    updatePool(activePool, "status", e.target.value)
                  }
            ...
            <input
              style={s.input}
              value={pool.label}
              onChange={(e) => updatePool(activePool, "label", e.target.value)}
            />
            ...
            <input
              ...
              value={pool.lockMsg}
              onChange={(e) =>
                updatePool(activePool, "lockMsg", e.target.value)
              }
            />
            ...
            <button style={s.addBtn} onClick={() => addNft(activePool)}>
              + Add NFT
            </button>
          </div>
        )}
    ...
    <div style={s.modalOverlay}>
      <div style={s.modalBox}>
        ...
        <input
          style={s.input}
          value={awardImgUrl}
          onChange={(e) => setAwardImgUrl(e.target.value)}
          placeholder="https://..."
        />
        <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
          ...
          <button
            style={s.btnConfirm}
            onClick={() => onConfirm({ winId, txHash, txUrl, awardImgUrl })}
          >
            Confirm ✓
          </button>
        </div>
...
const s = {
  ...
  loginBox: {
    ...
    margin: "80px auto",
    background: "#0F0F1A",
    border: "0.5px solid rgba(153,69,255,0.4)",
    borderRadius: 14,
    padding: 28,
    display: "flex",
  ...
  tabs: {
    ...
    gap: 6,
    marginBottom: 18,
    borderBottom: "0.5px solid rgba(153,69,255,0.18)",
    paddingBottom: 10,
    flexWrap: "wrap",
  },
  tab: {
  ...
  editor: {
    border: "0.5px solid rgba(153,69,255,0.18)",
    borderRadius: 8,
    padding: 16,
  },
  editorHeader: {
  ...
  btnConfirm: {
    ...
    border: "0.5px solid rgba(20,241,149,0.5)",
    color: "#14F195",
    padding: "8px 14px",
    borderRadius: 6,
    cursor: "pointer",
  ...
  addBtn: {
    background: "transparent",
    border: "0.5px dashed rgba(153,69,255,0.3)",
    color: "#888070",
    padding: 8,
    borderRadius: 7,
    cursor: "pointer",
  ...
  modalBox: {
    background: "#0F0F1A",
    border: "0.5px solid rgba(20,241,149,0.4)",
    borderRadius: 12,
    padding: 20,
    width: 320,
  },
